      ...
      <PanelCard title="메인 대시보드" subtitle="Daily Loop">
        ...
        <div className="pg-grid pg-gap-4 lg:pg-grid-cols-[1.2fr_0.8fr]">
          <div className="pg-rounded-2xl pg-border pg-border-white/10 pg-bg-white/[0.04] pg-p-4">
            ...
            <div className="pg-mt-2 pg-text-sm pg-leading-7 pg-text-slate-300">
              미션, 업적, 연구 완료가 모두 계정 성장으로 이어집니다. 실패가 있어도 진행은 계속 쌓입니다.
            </div>
          ...
          <div className="pg-rounded-2xl pg-border pg-border-white/10 pg-bg-white/[0.04] pg-p-4">
            <div className="pg-text-sm pg-font-medium pg-text-white">단기 지표</div>
            <div className="pg-mt-3 pg-grid pg-grid-cols-2 pg-gap-3">
              <Metric label="미션 완수율" value={formatPercent(computed.missionCompletionRate)} />
              <Metric label="도감 완성도" value={formatPercent(computed.collectionCompletionRate)} />
      ...
      <div className="pg-grid pg-gap-4 xl:pg-grid-cols-[1fr_1fr]">
        <PanelCard title="오늘의 운영 루프" subtitle="Daily / Weekly">
          <div className="pg-space-y-3">
            {missions.daily.map((mission) => {
              const definition = DAILY_MISSIONS.find((entry) => entry.id === mission.id);
              return (
                <div key={mission.id} className="pg-rounded-2xl pg-border pg-border-white/10 pg-bg-white/[0.04] pg-p-4">
                  <div className="pg-flex pg-items-center pg-justify-between">
                    <div className="pg-text-sm pg-font-medium pg-text-white">{definition?.label ?? mission.id}</div>
                    <div className="pg-text-xs pg-text-slate-400">{mission.claimed ? '완료' : `${mission.progress}/${definition?.target ?? 1}`}</div>
                  </div>
                  <ProgressBar value={mission.progress} max={definition?.target ?? 1} tone={mission.claimed ? 'lime' : 'cyan'} />
                </div>
              );
            })}
          </div>
        ...
        <PanelCard title="시즌 목표" subtitle="Season Zero">
          <div className="pg-rounded-2xl pg-border pg-border-violet-300/20 pg-bg-violet-300/10 pg-p-4">
            ...
            <div className="pg-mt-2 pg-text-sm pg-leading-6 pg-text-slate-300">
              연구, 원정, 강화 최고 기록을 바탕으로 시즌 보급품이 누적됩니다.
            </div>
          </div>
