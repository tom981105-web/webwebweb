...
export const useGameStore = create<StoreState>((set, get) => {
  ...
  return {
    ...
    initialize: () => {
      ...
      refreshSessionWindows(next, now, getDayKey(now), getWeekKey(now));
      const offlineSummary = createOfflineSummary(next, now - next.lastSavedAt);
    ...
    tick: (deltaMs) => {
      ...
      if (now - next.automation.lastAutomationAt >= 3500) {
        next.automation.lastAutomationAt = now;
